...
Not wired up yet, deliberately. When it is, the target is **Render**: this is a
persistent Express process plus a background worker for the outbox, which rules
out static and serverless hosts. One web service (`npm start`) and one background
worker (`npm run worker`). Note that SQLite on a container filesystem is
ephemeral — a deploy needs either a Render persistent disk or a switch to the
Supabase Postgres as primary.
